---
title: v1.15.0
description: "Rules can now do two things at once with a new optional ELSE branch — assign and revert roles with a single rule. Plus a brand-new Stripe Subscriber Role plugin, instant roles the moment you link a Twitch, YouTube, or Kick account, quiz grading and retakes for Forms, front-of-the-queue background sync for Premium, stackable subscriptions, and a major reliability upgrade for the bot."
...
Two related bugs could temporarily cap a **premium** role link at the free 100-user limit during a full resync — one in the API response plugins read, one in the plugins' fallback when that read briefly failed. Both are fixed: your full premium user limit is always respected, and a transient hiccup now simply retries instead of truncating your linked users.
